            ...
            if 'Player Name' in df.columns and 'Session Date' in df.columns:
                ...
                for _, row in df.iterrows():
                    ...
                    parsed_data.append({
                        ...
                        'aantal_deceleraties': int(row.get('Deceleration Z4 to Z6', row.get('Decelerations Z4 to Z6', row.get('Decelerations', 0)))),  # Deceleraties
        ...
        with st.form("rpe_input"):
            ...
            if st.form_submit_button("💾 RPE Data Opslaan", use_container_width=True):
                ...
                try:
                    ...
                    # Get next RPE ID using Supabase compatible query
                    try:
                        # Get max ID directly from Supabase
                        existing_rpe = client.table('rpe_data').select('rpe_id').order('rpe_id', desc=True).limit(1).execute()
                        if existing_rpe.data and len(existing_rpe.data) > 0:
                            next_id = existing_rpe.data[0]['rpe_id'] + 1
                        else:
                            next_id = 1
                    except:
                        # Fallback: just use a timestamp-based ID
                        import time
