...
st.title("📈 Individueel Overzicht")

if SUPABASE_MODE:
    # Supabase mode - modern approach
    st.info("🌐 Connected to Supabase database")
    
    # Test connection
    if not test_supabase_connection():
        st.error("❌ Cannot connect to Supabase. Please check configuration.")
        st.stop()
    ...
    if all_players.empty:
        st.warning("📭 Geen spelers gevonden. Voeg eerst spelers toe via Spelersbeheer.")
    else:
        ...
        viz_metric = st.radio("📊 Selecteer metric voor analyse", ["MAS (Aanbevolen)", "TrueVIFT", "VO2Max", "Alle"], horizontal=True)
        ...
        if len(speler_data) == 0:
            ...
            st.info(f"📊 **{selected_speler}** - Geen 30-15 testresultaten beschikbaar")
            
            col1, col2 = st.columns(2)
            
            with col1:
                st.markdown("#### 💪 Huidige Status")
                st.write("🔬 **30-15 Test:** Nog niet uitgevoerd")
                st.write("📈 **Fitness Level:** Onbekend")
                st.write("🎯 **Aanbeveling:** Plan een 30-15 test in")
                
                # Check andere beschikbare data
                st.markdown("#### 📊 Andere Beschikbare Data")
                
                # GPS data check
                gps_check = (lambda result: result[0] if result else None)(execute_db_query("""
                    SELECT COUNT(*) FROM gps_data WHERE speler = ?
                """, (selected_speler,)))[0]
                
                if gps_check > 0:
                    st.write(f"✅ GPS Data: {gps_check} sessies beschikbaar")
                else:
                    st.write("❌ GPS Data: Geen data")
                
                # RPE data check  
                rpe_check = (lambda result: result[0] if result else None)(execute_db_query("""
                    SELECT COUNT(*) FROM rpe_data WHERE speler = ?
                """, (selected_speler,)))[0]
                
                if rpe_check > 0:
                    st.write(f"✅ RPE Data: {rpe_check} sessies beschikbaar")
                else:
                    st.write("❌ RPE Data: Geen data")
            
            with col2:
                st.markdown("#### 🎯 Volgende Stappen")
                st.write("1. **30-15 Test Plannen**")
                st.write("   • Zorg voor goede weersomstandigheden")
                st.write("   • Plan test op begin van trainingsweek")
                st.write("   • Zorg voor adequate warming-up")
                st.write("")
                st.write("2. **Data Verzameling**")
                st.write("   • GPS tracker tijdens trainingen")
                st.write("   • RPE scores na elke sessie")
                st.write("   • Regelmatige herhaaltests")
                
                # Link naar andere pagina's
                st.markdown("#### 🔗 Gerelateerde Pagina's")
                st.write("• **Fysieke Training Analyse** voor GPS data")
                st.write("• **Spelersbeheer** voor profiel informatie")
                st.write("• **Team Compleet** voor vergelijking met team")
        else:
            ...
            with col2:
                ...
                st.metric("🎯 Laatste MAS", f"{laatste_test['TrueVIFT']:.1f} km/u")
                st.metric("🫁 Laatste VO2Max", f"{laatste_test['VO2MAX']:.1f} ml/kg/min")
                st.metric("🎂 Leeftijd", f"{laatste_test['Leeftijd']} jaar")
                st.metric("📅 Laatste test", laatste_test['Maand'])
                ...
                if len(speler_data) > 1:
                    ...
                    st.metric("📈 MAS Progressie", f"{verschil_mas:+.1f} km/u")
                    st.metric("📈 VO2Max Progressie", f"{verschil_vo2:+.1f} ml/kg/min")
            
            # Gedetailleerde tabel
            st.subheader("📋 Alle testresultaten")
            ...
            col_names = {
                "Maand": "Test Maand",
                "TrueVIFT": "MAS (km/u)",
                "MAS_Progressie": "MAS Δ (km/u)",
                "VO2MAX": "VO2Max (ml/kg/min)",
                "VO2_Progressie": "VO2Max Δ (ml/kg/min)",
                "Leeftijd": "Leeftijd"
            }
            ...
            st.download_button(
                label=f"📥 Download {selected_speler} gegevens",
                data=csv_individual,
