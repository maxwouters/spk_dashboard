...
# Header
st.markdown('<div class="main-header"><h1>👥 Team Overzicht</h1></div>', unsafe_allow_html=True)
...
st.markdown("### <span class='accent-blue'>📊 Team Statistieken</span>", unsafe_allow_html=True)

col1, col2, col3, col4 = st.columns(4)

# Totaal aantal spelers
try:
    alle_spelers = execute_db_query("""
    ...
    with col1:
        ...
        st.metric("👥 Totaal Spelers", totaal_spelers)
...
try:
    ...
    with col2:
        st.markdown('<div class="metric-card">', unsafe_allow_html=True)
        st.metric("🧪 Laatste Tests", latest_tests[0] if latest_tests else 0)
        st.markdown('</div>', unsafe_allow_html=True)
    
    with col3:
        st.markdown('<div class="metric-card">', unsafe_allow_html=True)
        st.metric("🏃‍♂️ Team MAS", f"{latest_tests[1]:.1f} km/u" if latest_tests and latest_tests[1] else "N/A")
        st.markdown('</div>', unsafe_allow_html=True)
    
    with col4:
        st.markdown('<div class="metric-card">', unsafe_allow_html=True)
        st.metric("🫁 Team VO2Max", f"{latest_tests[2]:.1f}" if latest_tests and latest_tests[2] else "N/A")
        st.markdown('</div>', unsafe_allow_html=True)
...
with col_left:
    st.markdown('<div class="info-card">', unsafe_allow_html=True)
    st.markdown("### <span class='accent-purple'>📈 Individuele Prestaties</span>", unsafe_allow_html=True)
    
    # Haal laatste 30-15 resultaten op
    try:
        resultaten = execute_db_query("""
        ...
        if resultaten:
            ...
            # Top performers
            st.markdown("#### 🏆 Top 5 MAS Prestaties")
            ...
            st.info("📭 Geen 30-15 testresultaten beschikbaar")
...
with col_right:
    st.markdown('<div class="info-card">', unsafe_allow_html=True)
    st.markdown("### <span class='accent-orange'>📊 Team Verdeling</span>", unsafe_allow_html=True)
    ...
    try:
        if 'df_resultaten' in locals() and not df_resultaten.empty:
            ...
            st.metric("👶 Jongste Speler", f"{df_resultaten['Leeftijd'].min()} jaar")
            st.metric("👴 Oudste Speler", f"{df_resultaten['Leeftijd'].max()} jaar") 
            st.metric("🎯 Gemiddelde Leeftijd", f"{df_resultaten['Leeftijd'].mean():.1f} jaar")
...
# ==================== GPS PRESTATIES ====================
st.markdown('<div class="info-card">', unsafe_allow_html=True)
st.markdown("### <span class='accent-green'>🏃‍♂️ GPS Training Prestaties</span>", unsafe_allow_html=True)
...
try:
    ...
    if gps_data:
        ...
        with gps_col1:
            ...
            st.metric("🏃 Gem. Totale Afstand", f"{avg_distance:.0f}m")
        
        with gps_col2:
            avg_hsr = df_gps['HSR'].mean()
            st.metric("⚡ Gem. High Speed", f"{avg_hsr:.0f}m")
        
        with gps_col3:
            max_speed = df_gps['Max_Speed'].max()
            st.metric("🚀 Hoogste Snelheid", f"{max_speed:.1f} km/u")
        
        with gps_col4:
            avg_sprints = df_gps['Sprints'].mean()
            st.metric("💨 Gem. Sprints", f"{avg_sprints:.0f}")
        ...
        st.info("📭 Geen GPS data beschikbaar")
...
st.markdown('<div class="info-card">', unsafe_allow_html=True)
st.markdown("### <span class='accent-blue'>📅 Aanwezigheid & Activiteit</span>", unsafe_allow_html=True)

try:
    # Recente aanwezigheid
    aanwezigheid = execute_db_query("""
    ...
    if aanwezigheid:
        ...
        with col_stat1:
            ...
            st.metric("📊 Gem. Aanwezigheid", f"{gem_aanwezigheid:.0f}%")
        
        with col_stat2:
            laatste_training = df_aanwezigheid.iloc[0]
            st.metric("📅 Laatste Training", f"{laatste_training['Aanwezig']}/{laatste_training['Totaal']}")
        
        with col_stat3:
            totaal_sessies = len(df_aanwezigheid)
            st.metric("🏃 Totaal Sessies", totaal_sessies)
    
    else:
        st.info("📭 Geen aanwezigheidsdata beschikbaar")
...
# ==================== QUICK INSIGHTS ====================
st.markdown('<div class="info-card">', unsafe_allow_html=True)
st.markdown("### <span class='accent-orange'>💡 Team Insights</span>", unsafe_allow_html=True)
...
with insights_col1:
    ...
    try:
        ...
        if 'df_resultaten' in locals() and not df_resultaten.empty:
            ...
            st.success(f"🏃‍♂️ Beste MAS: **{beste_mas['Speler']}** ({beste_mas['MAS']:.1f} km/u)")
            st.success(f"🫁 Beste VO2Max: **{beste_vo2['Speler']}** ({beste_vo2['VO2MAX']:.1f})")
            
            # Team gemiddelden vs normwaarden
            team_mas_avg = df_resultaten['MAS'].mean()
            if team_mas_avg > 16.0:  # Goede normwaarde voor voetbal
                st.success(f"⭐ Team MAS boven gemiddelde: {team_mas_avg:.1f} km/u")
...
with insights_col2:
    ...
    try:
        if 'df_resultaten' in locals() and not df_resultaten.empty:
            ...
            st.warning(f"📉 Laagste MAS: **{laagste_mas['Speler']}** ({laagste_mas['MAS']:.1f} km/u)")
            
            # Spreiding in team
            mas_std = df_resultaten['MAS'].std()
            if mas_std > 2.0:  # Grote spreiding
                st.warning(f"📊 Grote spreiding in team: {mas_std:.1f} km/u")
            
            # Aanwezigheid
            if 'df_aanwezigheid' in locals() and not df_aanwezigheid.empty:
                if df_aanwezigheid['Percentage'].mean() < 80:
                    st.warning("📅 Aanwezigheid kan beter (< 80%)")
...
with col_export1:
    if st.button("📊 Exporteer Team Data"):
        try:
...
with col_export2:
    st.info("💡 **Tip:** Gebruik dit overzicht voor team briefings en planning")
