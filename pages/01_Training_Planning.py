...
if not table_exists:
    ...
    if df.empty:
        ...
        # Clustering alleen op basis van de geselecteerde basis metric (niet afstand)
        kmeans = KMeans(n_clusters=3, random_state=42, n_init='auto')
        zones = zone_df.dropna(subset=[base_column]).drop_duplicates(subset=["Speler"]).copy()
        
        # Gebruik alleen de basis metric voor clustering (MAS, VIFT of VO2Max)
        zones["Groep"] = kmeans.fit_predict(zones[[base_column]])
