...
def safe_fetchdf(query: str, params: Dict = None) -> pd.DataFrame:
    ...
    try:
        ...
        if query.strip().upper().startswith('SELECT'):
            ...
            return handle_ddl_query(query, params)
...
def handle_insert_query(query: str, params: tuple = None) -> pd.DataFrame:
    """Handle INSERT queries by converting them to Supabase inserts"""
    try:
        supabase = get_supabase_client()
        
        # Parse INSERT query: INSERT INTO table_name (col1, col2, ...) VALUES (?, ?, ...)
        insert_match = re.search(r'INSERT\s+INTO\s+(\w+)\s*\(([^)]+)\)\s*VALUES\s*\([^)]+\)', query, re.IGNORECASE)
        
        if not insert_match:
            print(f"Could not parse INSERT query: {query}")
            return pd.DataFrame()
        ...
        if params and len(params) == len(columns):
            data = {}
            for i, col in enumerate(columns):
                # Skip sequences/auto-increment columns
                if 'nextval' not in query or col != columns[0]:
                    data[col] = params[i]
            
            # Remove any sequence columns (contact_id, etc)
            if 'nextval' in query and columns:
                first_col = columns[0]
        ...
        st.error(f"Insert query failed: {e}")
        print(f"Insert query failed: {e}, query: {query}, params: {params}")
        return pd.DataFrame()

def handle_ddl_query(query: str, params: tuple = None) -> pd.DataFrame:
    """Handle DDL queries (CREATE TABLE, CREATE SEQUENCE, etc.)"""
    # For DDL queries in Supabase, we mostly ignore them since tables should exist
    # Just return success to prevent errors
    print(f"DDL query ignored (tables managed in Supabase): {query[:100]}...")
    return pd.DataFrame([{"success": True}])
